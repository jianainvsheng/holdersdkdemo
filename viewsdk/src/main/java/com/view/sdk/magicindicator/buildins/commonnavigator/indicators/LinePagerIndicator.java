...
import com.view.sdk.magicindicator.FragmentContainerHelper;
...
public class LinePagerIndicator extends View implements IPagerIndicator {
    ...
    private ViewPager.OnPageChangeListener ml;
    ...
    protected void onDraw(Canvas canvas) {
        int size = UIUtil.dip2px(getContext(), 38);

        if(mLineRect.right - mLineRect.left > size){
            int every = (int)((float)(mLineRect.right - mLineRect.left - size) /2);
            mLineRect.right = mLineRect.right - every;
            mLineRect.left = mLineRect.left + every;
        }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if(ml != null){
            ml.onPageScrolled(position,positionOffset,positionOffsetPixels);
        }
    ...
    public void onPageScrollStateChanged(int state) {
        if(ml != null){
            ml.onPageScrollStateChanged(state);
        }
    }

    public void setOnViewPagerListener(ViewPager.OnPageChangeListener l){
